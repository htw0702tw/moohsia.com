...
補刀數（`lastHits` / `minions`）和治療量（`healing`）是不同欄位。發育分頁只顯示總經濟、野怪經濟、補刀數。治療量在團隊分頁。已發布的對局若把這兩欄對調，可用 `node scripts/rewrite-player-matches.mjs` 改寫匯出的站點 JSON：它不會新增對局。2026-09-25 22:59 娜塔亞 8/6/4 若已在檔案裡，會改成補刀 34、治療 6077。AOVRanking 還沒收錄的新對局，要在管理頁貼上展開後的歷史戰績再發布。目錄約 50 場；更早的對局只要已經存著就會留著。
