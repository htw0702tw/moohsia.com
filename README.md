...
AOVRanking 戰績表頭是 `補兵 | 控場 | 治療 | 塔傷`。那不是遊戲內分頁：補兵不是補刀數，控場是秒數不是控制效果。這四格存成 `rankingFarm`、`rankingControl`、`rankingHealing`、`rankingTower`，公開頁不把它們顯示成補刀數、控制效果、治療量、對塔傷害。遊戲內數字只在三種情況出現：表頭本來就是補刀數／治療量／控制效果／對塔傷害、管理頁勾了「已對過遊戲內畫面」並填上覆寫、或這場已是 2026-09-25 22:59 娜塔亞 8/6/4（補刀 34、治療 6077、控制 6534、對塔 2089）。`node scripts/rewrite-player-matches.mjs` 會把已存的控場秒數從補刀數挪走，不會新增對局。2026-09-25 對 `FightHistory/View?searchType=playerName&keyword=htw0702aov` 的抓取回 `aov_blocked`，沒有把 22:59 加進對局。那一場要等管理頁貼上展開後的歷史戰績；腳本只會改已經存在的同一列。目錄約 50 場；更早的對局只要已經存著就會留著。
