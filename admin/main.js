...
function aovImportPanel() {
  ...
  return `<section class="card aov-import${state.aovFocusPaste ? " is-paste" : ""}">
    ...
    <p class="hint">標題列有結果、KDA 與地圖。地圖上的「經典競技」、「競賽模式」和「冠軍賽」照原名保存，不會一律存成排位賽；記分板有排位積分變化才標成排位賽，地圖寫一般或一般對戰才標成一般。MapID_ 不會顯示。「傳說之巔」存成巔峰對決。結果會存成勝或敗。每一場的藍方、紅方記分板要等該場展開後才寫進頁面；英雄取玩家名稱旁的圖片，自己在哪一隊看名稱是否對上關鍵字。沒展開的場次，記分板只填自己的 KDA。再次匯入時，已展開的記分板、裝備和輸出不會被摺疊列蓋掉。若這次只展開了第一場，請再展開其餘場次，或上傳已展開記分板的 .html。</p>
    <div class="pair">
